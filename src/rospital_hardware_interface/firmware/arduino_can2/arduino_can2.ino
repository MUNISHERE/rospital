...
void setClosedLoopControl() {
  uint8_t closedLoopData[8] = {0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void sendCANMessage(uint8_t node_id, uint16_t command_id, uint8_t len, const uint8_t *data) {
  uint32_t can_id = (node_id << 5) | command_id;
  int retry = 3;
  while (retry--) {
    uint8_t sendStatus = CAN.sendMsgBuf(can_id, 0, len, const_cast<uint8_t*>(data));
...
void requestCANMessage(uint8_t node_id, uint16_t command_id) {
  uint32_t can_id = (node_id << 5) | command_id;
  CAN.sendMsgBuf(can_id, 0, 0, nullptr);
}

bool waitForCANMessage(uint8_t node_id, uint16_t command_id, unsigned long timeout_ms, uint8_t* data_out, uint8_t* len_out) {
  unsigned long startTime = millis();
...
void receiveCANMessage(unsigned long* id, uint8_t* len, uint8_t* data) {
  uint8_t status = CAN.checkReceive();
